...
# @dev Address of the RecursiveConditionalNFT contract that forms the condition for transferring and receiving this NFT
preconditionContractAddress: public(address)
...
def initialize(
	...
	@param _preconditionAddress Address of the RecursiveConditionalNFT contract that is condition for holding, transferring and receiving token. If ZERO_ADDRESS, set to None.
    ...
    self.preconditionContractAddress = _preconditionAddress
    self.tokenId = 0
    self.burntCount = 0
...
def _isEligible(_from: address) -> bool:
	"""
	@dev Returns 'True' if address has at least one RecursiveConditionalNFT deployed at preconditionContract,otherwise 'False'
	"""
	if self.preconditionContractAddress == ZERO_ADDRESS:
		return True

	return ERC721(self.preconditionContractAddress).balanceOf(_from) > 0

@view
@external
def isEligible(_from: address) -> bool:
	"""
	@dev Returns 'True' if address has at least one RecursiveConditionalNFT deployed at preconditionContract, otherwise 'False'
	"""
	return self._isEligible(_from)
...
def _transferFrom(_from: address, _to: address, _tokenId: uint256, _sender: address):
	...
	# Check if both sender and receiver are eligible
    assert self._isEligible(_from)
    assert self._isEligible(_to)
